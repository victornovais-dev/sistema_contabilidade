...
form.addEventListener("submit", async (event) => {
  ...
  const forcarTrocaSenha = Boolean(forcePasswordChangeInput?.checked);
  ...
  try {
    ...
    const enviarCriacao = async (tokenCsrf) =>
      fetch(adminUserApiBasePath, {
        method: "POST",
        headers: {
    ...
    let response = await enviarCriacao(tokenCsrf);
    if (response.status === 403) {
      tokenCsrf = await carregarCsrfToken(true);
      response = await enviarCriacao(tokenCsrf);
    }
    const data = await response.json().catch(() => ({}));
    ...
    if (response.status === 403) {
      showFeedback("error", "Acesso negado. Atualize a pagina e tente novamente.");
      return;
    }
    if (!response.ok) {
